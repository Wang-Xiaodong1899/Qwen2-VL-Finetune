...
from trl.trainer.rrhf_trainer import RRHFTrainer
...
class QwenRRHFTrainer(RRHFTrainer):

    def __init__(self, processing_class, *args, **kwargs):
        super(QwenRRHFTrainer, self).__init__(processing_class=processing_class, *args, **kwargs)
    ...
    def concatenated_forward(self, model: nn.Module, batch: dict[str, Union[list, torch.LongTensor]], is_ref_model: bool = False):
        ...
        if self.loss_type == "ipo" or self.loss_type == "simpo" or "simpo" in self.loss_type or self.loss_type == "rrhf" or "rrhf" in self.loss_type:
    ...
    def _save_checkpoint(self, model, trial):
        if self.args.lora_enable:
            ...
            super(QwenRRHFTrainer, self)._save_checkpoint(model, trial)