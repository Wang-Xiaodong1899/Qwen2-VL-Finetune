...
def get_frame_list_info(frame_list, min_pixels, max_pixels, width, height, fps, max_frames, min_frames):
    # Using this because of process_vision_info function
    # Need to fix this in the future
    # transfer video_path to a list of frames
...
def sample_elements(input_list=[], n=48):
    length = len(input_list)
    
    if length <= n:
        return input_list.copy()  # 如果长度小于等于128，直接返回副本
    ...
    for i in range(n):
        # 计算索引并取整
        index = int(i * step)
        selected.append(input_list[index])
    return selected

class SupervisedFramesDataset(Dataset):
    """Dataset for supervised fine-tuning."""
